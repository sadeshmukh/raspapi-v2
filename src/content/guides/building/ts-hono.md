...
To set up D1, we need to configure Cloudflare Workers to let it know we need a D1 database. This is done with the `wrangler.jsonc` file, which should have been generated by `bun create` in the previous guide. When you open this file, there should be a commented out section like the following:
...
To add a database, uncomment the section and change the `database_name` to something recognizable, like `"hono-reminders"`. You can leave `database_id` and `binding` alone. Finally, fix the commas in the file (you probably need to add a comma before this section and remove the one at its end).

After our changes, we need to let TypeScript know about our new database by running this command: `bun cf-typegen`. This will generate the required TypeScript declarations that let us use your D1 database in code.
